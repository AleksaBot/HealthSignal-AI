...
function ReportDataRow({ label, value }: { label: string; value: string | null }) {
  if (!value) return null;
  return (
    <div className="rounded-xl border border-slate-200/90 bg-white/90 px-3 py-2 dark:border-slate-700 dark:bg-slate-900/70">
...
function SectionCard({ title, icon, children }: { title: string; icon?: ReactNode; children: ReactNode }) {
  return (
    <section className="animate-fade-up self-start space-y-2.5 rounded-2xl border border-slate-200/80 bg-white/85 p-3.5 transition duration-300 dark:border-slate-700/80 dark:bg-slate-900/65 sm:p-4">
            ...
            <header className="space-y-3 rounded-2xl border border-brand-200/70 bg-gradient-to-br from-white/95 via-white/90 to-brand-50/80 p-4 shadow-sm shadow-brand-200/25 dark:border-brand-500/30 dark:from-slate-900/80 dark:via-slate-900/70 dark:to-brand-950/30 dark:shadow-brand-900/20">
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <div className="flex w-full flex-col items-start gap-2 sm:w-auto sm:items-end">
                  <Link
                    href="/symptom-analyzer"
                    className="inline-flex items-center gap-1 rounded-lg border border-brand-300/70 bg-brand-50/90 px-3 py-1.5 text-xs font-semibold text-brand-800 transition hover:-translate-y-0.5 hover:border-brand-400 hover:bg-brand-100 dark:border-brand-400/50 dark:bg-brand-900/30 dark:text-brand-100 dark:hover:border-brand-300"
                  >
                    <span aria-hidden>➕</span>
                    Start New Check
                  </Link>
              ...
              <div className="grid items-start gap-2.5 sm:gap-3 lg:grid-cols-2">
                <SectionCard title="Risk & Triage" icon={<span aria-hidden>🩺</span>}>
                  <div className="space-y-2">
                    ...
                    <p className="text-[11px] text-slate-500 dark:text-slate-400">Based on completeness of available answers.</p>
                ...
                <SectionCard title="Doctor Visit Prep" icon={<span aria-hidden>🗂️</span>}>
                  <ul className="space-y-2">
                    {selectedVisitPrep.map((item) => (
                      <li key={item} className="rounded-lg border border-slate-200/80 bg-white/95 px-2.5 py-1.5 text-sm leading-snug text-slate-700 dark:border-slate-700 dark:bg-slate-900/70 dark:text-slate-200">
                        {item}
                      </li>
                    ))}
                  </ul>
